...
const client = new Web3Storage({ token: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJkaWQ6ZXRocjoweDBiNTlBMTg4NzdmYmRlMjhlZjUzNUJlQTg1MzBGMjlDYmFDMkY1QzAiLCJpc3MiOiJ3ZWIzLXN0b3JhZ2UiLCJpYXQiOjE2NjMxNTUwNDA3NDEsIm5hbWUiOiJTYWZpcGF5In0.WZXiF2KM9ykaScPS-HpTkHIumL8zGWSAVdS9uysMgDo" });
...
const KYCDashboard = () => {
  ...
  const handleSaveKYCInfo = async() => {
    ...
    if(selectedFile){
      if(confirm("Are you sure you want to save this information?")){
        try{
          const fileInput = document.querySelector('input[type="file"]');
          setMsgUploadingIPFS("Encrypting and uploading to file the IPFS network...");
          const rootCid = await client.put([selectedFile]);
          setMsgUploadingIPFS(`File CID ${rootCid}`);
          kyc_info['ID_CID'] = rootCid;
          const data = await serverInstance.putRequest("user/kyc/set", kyc_info, true);
          if(data && data._id){
            localStorage.user = JSON.stringify(data);
            alert("KYC information saved successfully.\nYou will be informed when your KYC is approved.");
          }
        }catch(e){
          setMsgUploadingIPFS("Failed to upload the file to IPFS network, please try again.");
        }      
      }
    }else {
  ...
  const fields = [
    ...
    { placeholder: "Legal middle name ", label: "Legal middle name",name : "middle_name" },
    { placeholder: "Enter your legal last name ", label: "Legal last name",name : "last_name"  },
    { placeholder: "Enter your birthdate ", label: "Birthdate",name : "birthdate"  },
    { placeholder: "Enter your place of birth ", label: "Place of birth",name : "place_of_birth" },
    { placeholder: "Enter your nationality", label: "Nationality",name : "nationality"  },
    { placeholder: "Enter your legal ID number", label: "ID number",name : "id_number"  },
    {
      placeholder: "Enter the country of issue of your ID",
      label: "Country of issue"
      ,name : "country_of_issue" 
    },
    { placeholder: "Enter your address ", label: "Address line 1 ",name : "address_line_1"  },
    { placeholder: "Enter your city", label: "City",name : "city"  },
    { placeholder: "Enter your state or province", label: "State/province",name : "province"  },
        ...
        <div className="w-4/5 form-content">
          {fields.map((f, i) => (
            <InputField
              ...
              value={user_info.kyc_info && user_info.kyc_info[f.name] ? user_info.kyc_info[f.name] : ""}
          ...
          <div style={{position:"relative"}} onClick={handleIDUpload} className="id-photo-wrapper w-full flex justify-center items-center">
            ...
            <input type="file" onChange={handleFileAdded} name="addingFile" style={{display: "none"}} ref={inputFile}/>
          </div>
